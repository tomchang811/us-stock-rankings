...
- **EVERY `gemini-2.5-flash` call needs `thinkingConfig: { thinkingBudget: 0 }` — including the structured (`responseSchema`) theme call, not just the grounded ones.** Default thinking eats the output-token budget → empty/truncated response → call fails. `enrichWithGemini` (structured, in `core.mjs`) once shipped *without* it and intermittently returned empty → caught → `aiSource:"none"` + SIC-fallback themes while the grounded `marketBriefing` (which had `thinkingBudget:0`) still succeeded — a confusing "GEMINI_API_KEY not set" UI message even though the key was fine. All Gemini calls now set `thinkingConfig:{thinkingBudget:0}` + a generous `maxOutputTokens` (4096 grounded, 8192 for the 50-ticker theme call). Output size also scales with item count — `explainNewEntrants` truncated (all reasons blank) when a day had **8 new entrants** at 2048 tokens; mitigated by 4096 + a tolerant `parseJsonObjects()` (tries whole-array parse, else regex-extracts complete `{…}` objects so a truncated tail still yields partial results). Keep both on any new grounded call.
...
- **`streak` has two producers that must be re-stitched: `backfill.mjs` chains within its window; the daily `snapshot.mjs` does `live-prev + 1`.** When a backfill overwrites the latest day, the two disagree at the seam. Fix = `scripts/reconcile-streak.mjs` (offline, idempotent): forward-chains `streak`/`rankChange`/`isNew` across `public/history/*.json` keeping the first day as baseline, then syncs the latest day into `rankings.json` — touching only those 3 fields, never the AI fields. It runs as a step in `backfill.yml` after a backfill. `backfill.mjs` also **preserves a production day's grounded AI** (`marketBriefing`/`themeSummary`/`newEntrants` + per-row themes) when overwriting a file whose `backfilled !== true`, so re-running backfill never wipes a real day's 市場焦點/發動題材.
