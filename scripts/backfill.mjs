// 回補歷史快照：抓取過去 N 個交易日的成交值前 50 名個股，寫入 public/history/<交易日>.json，
// 並重建 index.json / trends.json。一次性／偶爾手動執行，用來把歷史走勢「補齊」。
//
// 用法： node scripts/backfill.mjs [N]      （N＝回補的交易日數，預設 60）
// 環境變數 / .env.local：POLYGON_API_KEY（必要）、GEMINI_API_KEY（選用）
//
// 與每日 snapshot 的差異：
//  - isNew / rankChange / streak 由「回補序列中相鄰的前一交易日」推算（非線上前一份）。
...
/** 從昨天往回收集 count 個「有資料」的交易日 grouped 結果（新到舊）。 */
async function collectTradingDays(key, count) {
  const days = [];
  const cursor = new Date();
  cursor.setUTCDate(cursor.getUTCDate() - 1);
  // 最多回看 count*2 個日曆日（含週末/假日緩衝）。
  for (let i = 0; i < count * 2 + 20 && days.length < count; i++) {
    const dow = cursor.getUTCDay();
    if (dow !== 0 && dow !== 6) {
      const ds = fmtDate(cursor);
      const data = await getJson(grouped(key, ds));
      if ((data.resultsCount ?? 0) > 0) {
        days.push({ date: ds, results: data.results });
        console.log(`  取得 ${ds}（${data.results.length} 檔）`);
      } else {
        console.log(`  ${ds} 無資料（假日或未開放），略過`);
      }
    }
    cursor.setUTCDate(cursor.getUTCDate() - 1);
  }
...
async function main() {
  ...
  if (!polygonKey) {
    console.error("未找到 POLYGON_API_KEY（本機請設於 .env.local）");
    process.exit(1);
  }
  const geminiKey = await readKey("GEMINI_API_KEY");

  console.log(`回補最近 ${n} 個交易日（多抓 1 日作漲跌幅/新進榜基準）…`);
  // 多收一日作為最舊輸出日的「前一交易日」基準（漲跌幅與 isNew/streak 起點）。
  const daysDesc = await collectTradingDays(polygonKey, n + 1);
  ...
  // 逐日排名 + 補明細（pickTopStocks 會就地更新並存快取）。
  const perDay = []; // [{ date, picked }]
  for (let i = 0; i < daysAsc.length; i++) {
    const day = daysAsc[i];
    const prevMap = new Map();
    if (i > 0) for (const b of daysAsc[i - 1].results) prevMap.set(b.T, b);
    console.log(`排名 ${day.date} …`);
    const picked = await pickTopStocks(day.results, prevMap, cache, polygonKey, { log: false });
    perDay.push({ date: day.date, picked });
  }
  ...
  for (const { picked } of perDay) {
    for (const p of picked) {
      if (!metaBySymbol.has(p.b.T)) {
        metaBySymbol.set(p.b.T, { name: p.meta.name ?? p.b.T, sector: formatSector(p.meta.sector ?? "—") });
      }
    }
  }
  if (geminiKey && metaBySymbol.size > 0) {
    ...
    try {
      ...
      console.warn(`Gemini 題材標註失敗，改用快取/SIC 後備：${e.message}`);
    }
  } else {
    console.log("未設定 GEMINI_API_KEY，題材使用快取/SIC 後備");
  }

  const themeOf = (t, sector) => {
    if (themesByTicker.get(t)) return themesByTicker.get(t);
  ...
  // 是否有實際題材（Gemini 本次標註 or 快取未過期）；否則才算 aiSource="none"。
  const haveThemes =
  ...
  for (let i = 0; i < perDay.length; i++) {
    ...
    if (i < startIdx) continue; // 基準日，不輸出

    const out = {
      rows,
      asOf: new Date(`${date}T20:00:00Z`).toISOString(),
